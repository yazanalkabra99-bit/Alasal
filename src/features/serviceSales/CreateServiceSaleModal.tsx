...
type OfficeOpt = { id: number; name: string; phone?: string | null };

export function CreateServiceSaleModal({
  open,
  onClose,
  onCreated,
}: {
  ...
  // Billing type: customer or office
  ...
  async function handleSubmit() {
    if (forWhom === 'customer' && !customerName.trim()) return setError('اسم العميل مطلوب');
    if (forWhom === 'office' && !officeId) return setError('يجب اختيار المكتب');
    ...
    try {
      ...
      if (forWhom === 'customer') {
        ...
        payload.office_party_id = Number(officeId);
      }

      const res = await api.post('/service-sales', payload);
      const saleId = Number(res.data.data.id);
      ...
      for (const f of files) {
        ...
        await api.post(`/service-sales/${saleId}/attachments`, fd, {
      ...
      setError(e?.response?.data?.error || 'فشل إنشاء عملية بيع الخدمة');
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    ...
    >
      <div className="space-y-4">
        ...
        <div className="p-3 rounded-xl bg-slate-800/30 border border-slate-700/50 space-y-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div className="relative md:col-span-2">
              ...
              <div className="mt-1 text-[11px] text-slate-500">الاقتراحات تظهر بعد أول حرفين (لتكون العملية مريحة).</div>
            </div>
            ...
            <div className="md:col-span-2">
              ...
              <div className="mt-1 text-[11px] text-slate-500">اختياري: اختر المكتب/المورد الذي تشتري منه الخدمة (ستنزل التكلفة بكشف حسابه)</div>
            </div>
          </div>
        </div>
        ...
        {/* Billing Party */}
        <div className="p-3 rounded-xl bg-slate-800/30 border border-slate-700/50 space-y-3">
          <h4 className="text-xs font-bold text-amber-400">معلومات الفوترة</h4>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div>
              <label className="text-xs text-slate-400 mb-1 block">لصالح</label>
              <Select
                value={forWhom}
                onChange={(e) => {
                  const v = e.target.value as 'customer' | 'office';
                  setForWhom(v);
                  setOfficeId('');
                  setCustomerName('');
                  setCustomerPhone('');
            ...
            {forWhom === 'office' && (
              <div>
                <label className="text-xs text-slate-400 mb-1 block">اختر المكتب *</label>
                <Select value={officeId} onChange={(e) => setOfficeId(e.target.value ? Number(e.target.value) : '')}>
