...
export function ExternalTicketCollectModal({
  open,
  onClose,
  ticketId,
  onSaved,
  ...
  const remainingUsd = Math.max(0, Number(summary?.remaining_sell_usd || 0));
  ...
  async function save() {
    ...
    setError(null);
    try {
      await api.post(`/external-tickets/${ticketId}/collect`, {
      ...
      setError(e?.response?.data?.error || 'فشل حفظ التحصيل');
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <Modal open={open} onClose={onClose} title="تحصيل تذكرة خارجية" width="max-w-xl">
      {error && (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          />
          {amountTooHigh && (
            <div className="text-[11px] text-rose-300 mt-1">المبلغ أكبر من المتبقي</div>
          )}
        </div>

        <div className="md:col-span-2">
          <div className="text-xs text-slate-400 mb-1">ملاحظة (اختياري)</div>
          <Input value={note} onChange={(e) => setNote(e.target.value)} placeholder="مثال: دفعة أولى" />
        </div>
      </div>
      ...
      <div className="flex items-center justify-end gap-3 mt-6">
        ...
        <Button
          onClick={save}
          disabled={loading || loadingMeta || !accountId || !currencyCode || !receiptNo.trim() || !(amount > 0) || amountTooHigh}
        >
          {loading ? 'جاري الحفظ…' : 'حفظ التحصيل'}
        </Button>
      </div>
    </Modal>
  );
