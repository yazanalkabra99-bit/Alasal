...
export function ExternalTicketSourcePaymentModal({
  open,
  onClose,
  ticketId,
  onSaved,
  ...
  const remainingUsd = Math.max(0, Number(summary?.remaining_buy_usd || 0));
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        setError(e?.response?.data?.error || 'تعذر تحميل بيانات السداد');
      } finally {
  ...
  async function save() {
    ...
    setError(null);
    try {
      await api.post(`/external-tickets/${ticketId}/pay-source`, {
  ...
  return (
    <Modal open={open} onClose={onClose} title="سداد مصدر التذكرة الخارجية" width="max-w-xl">
      {error && (
      ...
      {summary && (
        <div className="mb-3 rounded-2xl border border-slate-800/60 bg-slate-900/30 p-3">
          <div className="text-xs text-slate-400">المتبقي للسداد للمصدر</div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          <div className="text-xs text-slate-400 mb-1">عملة السداد</div>
          ...
          <div className="text-[11px] text-slate-500 mt-1">المتبقي بهذه العملة: {currencyCode ? fmtMoney(maxAmount || 0, currencyCode) : '—'}</div>
        </div>
        ...
        <div>
          ...
          <Input
            ...
            min={0}
          />
          {amountTooHigh && <div className="text-[11px] text-rose-300 mt-1">المبلغ أكبر من المتبقي</div>}
        </div>

        <div className="md:col-span-2">
          <div className="text-xs text-slate-400 mb-1">ملاحظة (اختياري)</div>
          <Input value={note} onChange={(e) => setNote(e.target.value)} placeholder="مثال: دفعة للمصدر" />
        </div>
      </div>
