...
export function ExternalTicketVoidModal({
  open,
  onClose,
  ticketId,
  onSaved,
  ...
  async function save() {
    ...
    setError(null);
    try {
      await api.post(`/external-tickets/${ticketId}/void`, {
        reason: reason || null,
        note: note || null,
  ...
  return (
    <Modal open={open} onClose={onClose} title="VOID — إلغاء تذكرة خارجية" width="max-w-xl">
      {error && (
      ...
      <div className="rounded-2xl border border-amber-800/60 bg-amber-950/20 p-3 text-xs text-amber-200">
        ...
        <div>
          VOID سيقوم بإلغاء التذكرة وتصفير مبالغ الشراء.
          إذا حددت غرامة، يبقى مبلغ الغرامة على العميل كربح تذكرة.
          إذا لم تحدد غرامة، يصبح إجمالي التذكرة = 0.
        </div>
      </div>
      ...
      <div className="mt-4 grid grid-cols-1 gap-3">
        <div>
          ...
          <Input value={reason} onChange={(e) => setReason(e.target.value)} placeholder="مثال: خطأ في البيانات" />
        </div>
        <div>
