...
import { Search, Plus, Filter, FileText, Clock, AlertTriangle, CheckCircle2, XCircle, Archive, RefreshCw, ChevronLeft, User, Calendar, DollarSign } from 'lucide-react';
import { api } from '../utils/api';
import { Badge } from '../components/ui/Badge';
import { Button } from '../components/ui/Button';
import { Input } from '../components/ui/Input';
import { Card } from '../components/ui/Card';
import type { PassportRequestRow, PassportStatus } from '../utils/types';
import { fmtMoney, fmtDate, daysLeft, passportStatusTone, passportStatusLabel } from '../utils/format';
import { useAuth, hasAnyRole } from '../state/auth';
import { CreatePassportModal } from '../features/passport/CreatePassportModal';

const statuses: PassportStatus[] = ['submitted','processing','ready','delivered','rejected','cancelled'];
...
export function PassportListPage() {
  ...
  const { user } = useAuth();
  const [rows, setRows] = useState<PassportRequestRow[]>([]);
  ...
  function partyTypeLabel(t: PassportRequestRow['billing_party_type']) {
    if (t === 'office') return 'مكتب شريك';
        return 'عميل';
  }
  ...
  async function load() {
    ...
    try {
      ...
      const res = await api.get('/passport-requests', { params });
      setRows(res.data.data || []);
    } catch (e: any) {
      setError(e?.response?.data?.error || 'تعذر تحميل طلبات الجوازات');
    } finally {
      setLoading(false);
    }
  }

  // One load on mount + whenever filters change (avoid double request on first render).
  useEffect(() => { load(); }, [status, special]);

  const counts = useMemo(() => {
    const c: Record<string, number> = {};
    for (const r of rows) c[r.passport_status] = (c[r.passport_status] || 0) + 1;
    return c;
  }, [rows]);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div className="animate-fade-in">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-black text-white">طلبات الجوازات</h1>
        ...
        <div className="flex items-center gap-2 animate-fade-in" style={{ animationDelay: '0.1s' }}>
          ...
          {hasAnyRole(user, 'employee', 'passport_admin', 'admin') && (
      ...
      <div className="flex flex-wrap items-center gap-2 animate-fade-in" style={{ animationDelay: '0.2s' }}>
        <div className="flex items-center gap-1 p-1 bg-slate-800/50 rounded-xl">
          ...
          {hasAnyRole(user, 'passport_admin', 'admin') && (
            <button
        ...
        <div className="flex flex-wrap gap-1.5">
          ...
          {statuses.map((s) => {
            ...
            return (
              ...
              >
                {Icon && <Icon size={12} />}
                {passportStatusLabel(s)} ({counts[s] || 0})
              </button>
            );
          })}
        </div>
      ...
      {loading ? (
        ...
        <div className="glass-card rounded-xl p-12 text-center animate-fade-in">
          ...
          <p className="text-sm text-slate-400 mb-4">ابدأ بإضافة طلب جواز جديد</p>
          {hasAnyRole(user, 'employee', 'passport_admin', 'admin') && (
            <Button variant="gradient" onClick={() => setCreateOpen(true)}>
              <Plus size={16} />
              طلب جديد
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {rows.map((r, index) => {
            ...
            const disp = (r.display_status || r.passport_status) as any;
            ...
            return (
              <div
                key={r.passport_request_id}
                onClick={() => navigate(`/passport/${r.passport_request_id}`)}
                className={`
              ...
              >
                ...
                <div className="flex items-start justify-between mb-3">
                  <div className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-medium ${statusColors[disp]}`}>
                    <Icon size={12} />
                    {passportStatusLabel(disp)}
                  </div>
                  <span className="text-xs text-slate-500">#{r.passport_request_id}</span>
                </div>
                ...
                <div className="flex items-center gap-2 mb-3 text-sm">
                  ...
                  <span className="text-slate-300">{r.passport_type_name}</span>
                </div>
      ...
      <CreatePassportModal
        ...
        onCreated={(id) => navigate(`/passport/${id}`)}
      />
    </div>
  );
}
