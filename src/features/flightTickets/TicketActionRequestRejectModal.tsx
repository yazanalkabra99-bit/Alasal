...
export function TicketActionRequestRejectModal({
  open,
  requestId,
  ticketId,
  actionType,
  ...
  async function submit() {
    ...
    setError(null);
    try {
      await api.post(`/flight-tickets/action-requests/${requestId}/reject`, { reason, note });
      onSaved();
      onClose();
    } catch (e: any) {
  ...
  return (
    <Modal open={open} title={`رفض طلب ${actionLabel}`} onClose={onClose} width="max-w-lg">
      <div className="space-y-3">
        ...
        <div className="text-sm text-slate-300">
          سيتم رفض الطلب وإشعار الموظف. (التذكرة: #{ticketId})
        </div>
        ...
        <div>
          ...
          <input
            ...
            placeholder="مثال: لا يمكن الاسترجاع / لا يمكن VOID…"
          />
        </div>
